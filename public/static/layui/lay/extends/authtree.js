	...
	obj = {
		...
		renderAuth: function(tree, dept, inputname){
			...
			layui.each(tree, function(index, item){
				...
				str += '<div><div class="auth-status"> '+(hasChild?'<i class="layui-icon auth-icon" style="cursor:pointer;float: left;margin: 10px 6px 0 0;"></i>':'')+(dept > 0 ? '<span style="float: left;margin: 10px 6px 0 0;">├─ </span>':'')+'<input type="checkbox" name="'+inputname+'" title="'+item.name+'" value="'+item.value+'" lay-skin="primary" lay-filter="checkauths" '+checked+'> </div> <div class="auth-child" style="display:none;padding-left:40px;"> '+append+'</div></div>'
			});
			str += '</div>';
			return str;
		}
