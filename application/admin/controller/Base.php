...
class Base extends Controller
{
    ...
    public function initialize()
    {
        ...
        if($admin['groupid']==1){
            ...
            $group_info =$group_model->where('id',$admin['groupid'])->find();
            $nowMenu = Db::name('menu')->where('url', $node)->where('id','in',$group_info['limits'])->order('sort ASC')->find();
            if(!$nowMenu){
                $this->error('您没有权限操作！');
            }
            $t_menu = Db::name('menu')->where('pid', 0)->where('id','in',$group_info['limits'])->order('sort ASC')->select();
            $allMenu = Db::name('menu')->where('id','in',$group_info['limits'])->order('sort ASC')->select();
        }
        $this->groupid =$admin['groupid'];
        //------------------------
        $t_menu = Db::name('menu')->where('pid', 0)->order('sort ASC')->select();
        $allMenu = Db::name('menu')->order('sort ASC')->select();
        $nowMenu = Db::name('menu')->where('url', $node)->order('sort ASC')->find();
