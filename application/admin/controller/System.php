...
class System extends Base
{
    ...
    public function AdminMember()
    {
        ...
        if($admin){
            foreach ($admin as $key=>$val){
                $group_info =$group_model->where('id',$val['groupid'])->find();
                $admin[$key]['group_name'] =$group_info['name'];
            }
        }
        //---------------------
    ...
    public function addAdminMember()
    {
        if (Request::isAjax()) {
             ...
             */
            $group_model =new \app\common\model\Group();
            $group_list =$group_model->where('status',1)->select()->toArray();
