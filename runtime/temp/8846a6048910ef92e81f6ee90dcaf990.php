<?php /*a:3:{s:51:"F:\phpstudy\WWW\rhaphp\themes/pc/mp/mp\special.html";i:1609835002;s:61:"F:\phpstudy\WWW\rhaphp\themes/pc/mp/..\admin\common\base.html";i:1609832566;s:62:"F:\phpstudy\WWW\rhaphp\themes/pc/mp/common\autoreplay_nav.html";i:1609726646;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<form class="layui-form" action="" style="padding: 20px 200px 30px 0;">
    ...
    <div class="layui-form-item">
        <label class="layui-form-label">卡券事件</label>
        <div id="replyType_card" class="layui-input-block">
            <input type="radio" <?php if($card['event'] == 'nocol'): ?> checked <?php endif; ?>  name="card" value="nocol" title="不处理">
            <input type="radio" <?php if(!(empty($card['keyword']) || (($card['keyword'] instanceof \think\Collection || $card['keyword'] instanceof \think\Paginator ) && $card['keyword']->isEmpty()))): ?> checked <?php endif; ?> name="card" value="keyword" title="触发关键词" >
            <input type="radio" <?php if(!(empty($card['addon']) || (($card['addon'] instanceof \think\Collection || $card['addon'] instanceof \think\Paginator ) && $card['addon']->isEmpty()))): ?> checked <?php endif; ?> name="card" value="addon" title="触发应用">
        </div>
    </div>
    <div id="tapNodecard">
        <div class="layui-tab-content card">
            <div class=" image_text layui-tab-item  <?php if($card['event'] == 'nocol'): ?> layui-show <?php endif; ?>">
            <div class="layui-form-item layui-form-text">
            </div>
        </div>
        <div class="card_keyword layui-tab-item <?php if(!(empty($card['keyword']) || (($card['keyword'] instanceof \think\Collection || $card['keyword'] instanceof \think\Paginator ) && $card['keyword']->isEmpty()))): ?> layui-show <?php endif; ?>">
        <div class="layui-form-item">
            <label class="layui-form-label"></label>
            <div class="layui-input-block">
                <input type="text" name="card_keyword" value="<?php echo htmlentities($card['keyword']); ?>" placeholder="请输入关键词" autocomplete="off" class="layui-input">
            </div>
        </div>
    </div>
    <div class="card_addon layui-tab-item <?php if(!(empty($card['addon']) || (($card['addon'] instanceof \think\Collection || $card['addon'] instanceof \think\Paginator ) && $card['addon']->isEmpty()))): ?> layui-show  <?php endif; ?>">
    <div class="layui-form-item">
        <label class="layui-form-label"></label>
        <div class="layui-input-block">
            <select name="card_addons">
                <option value="">请选择应用</option>
                <?php if(is_array($addons) || $addons instanceof \think\Collection || $addons instanceof \think\Paginator): $i = 0; $__LIST__ = $addons;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
                <option <?php if($card['addon'] == $v['addon']): ?> selected <?php endif; ?>  value="<?php echo htmlentities($v['addon']); ?>"><?php echo htmlentities($v['name']); ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        $("#replyType_card").click(function(){
            var class_a='card_'+ $("input[name='card']:checked").val();
            $('.card div').removeClass('layui-show')
            $('.'+class_a).addClass('layui-show')
        });
