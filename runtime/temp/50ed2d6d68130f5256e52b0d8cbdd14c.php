<?php /*a:2:{s:62:"F:\phpstudy\WWW\rhaphp\themes/pc/mp/website\category_list.html";i:1609843143;s:61:"F:\phpstudy\WWW\rhaphp\themes/pc/mp/..\admin\common\base.html";i:1609832566;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<a href="<?php echo url('categoryadd'); ?>" class="layui-btn layui-btn-normal layui-btn-sm rha-nav-title">增加栏目</a>
</h2>
        </div>
        <?php endif; ?>
...
<table class="layui-table">
    ...
    <tbody>
    <?php if(is_array($categoryList) || $categoryList instanceof \think\Collection || $categoryList instanceof \think\Paginator): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
        <tr>
            ...
            <td><?php if($v['icon']): ?><img src="<?php echo htmlentities($v['icon']); ?>" height="28" /><?php endif; ?></td>
            <td><?php if($v['show']): ?>显示<?php else: ?>隐藏<?php endif; ?></td>
            <td><a class="rha-bt-a" href="<?php echo url('categoryupdate',['id'=>$v['id']]); ?>">修改</a> <a href="javascript:;" onclick="delMenu('<?php echo htmlentities($v['id']); ?>')" class="rha-bt-a" >删除</a></td>
        </tr>
    <?php endforeach; endif; else: echo "" ;endif; ?>
    </tbody>
</table>
    ...
    function delMenu(id) {
        layui.use('layer', function(){
            ...
            layer.confirm('删除操作可能引起系统不能正常使用，你确定需要删除吗？', {
                ...
                $.post("<?php echo url('Website/delCategory'); ?>",{'id':id},function (res) {
                    if(res.status==1){
...
layui.use('form', function(){
    ...
    form.on('submit(updateSort)', function(data){
        $.post("<?php echo url('Website/updateSort'); ?>",data.field,function (res) {
            if(res.status=='0'){
                layer.msg(res.msg);
            }
            if(res.status=='1'){
